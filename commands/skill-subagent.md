---
description: 설치된 스킬을 쓰는 위임. CLAUDE.md·MCP는 없다. `/orbit:skill-subagent` 커맨드가 사용한다.
argument-hint: "[모델명] [--parallel-count N] <프롬프트>"
---

# /orbit:skill-subagent 위임

`/orbit:skill-subagent [모델명] [--parallel-count N] <프롬프트>` — 서브에이전트 유형 `orbit:skill-subagent`을 띄운다. 접근 범위: 스킬 사용 가능 — CLAUDE.md·MCP 없음. 하위 위임(Agent 도구) 불가 — 여러 에이전트로 나누는 일은 메인 세션이 한다.
...
2. 같은 턴에 `Agent` 도구로 띄운다. 개수가 N이면 N개를 한 턴에 병렬로:
   ...
   - `subagent_type`: `orbit:skill-subagent`
   - `model`: 파싱한 모델
   - `prompt`: 프롬프트 원문
