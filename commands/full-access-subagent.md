---
description: 슈퍼바이저와 같은 접근의 위임(CLAUDE.md·스킬·MCP·전 도구). 무겁다 — 꼭 필요할 때만. `/orbit:full-access-subagent` 커맨드가 사용한다.
argument-hint: "[모델명] [--parallel-count N] <프롬프트>"
---

# /orbit:full-access-subagent 위임

`/orbit:full-access-subagent [모델명] [--parallel-count N] <프롬프트>` — 서브에이전트 유형 `orbit:full-access-subagent`을 띄운다. 접근 범위: 전체 접근(슈퍼바이저급).
...
2. 같은 턴에 `Agent` 도구로 띄운다. 개수가 N이면 N개를 한 턴에 병렬로:
   ...
   - `subagent_type`: `orbit:full-access-subagent`
   - `model`: 파싱한 모델
   - `prompt`: 프롬프트 원문
