---
description: MCP 도구를 쓰는 위임. CLAUDE.md·스킬은 없다. `/orbit:mcp-subagent` 커맨드가 사용한다.
argument-hint: "[모델명] [--parallel-count N] <프롬프트>"
---

# /orbit:mcp-subagent 위임

`/orbit:mcp-subagent [모델명] [--parallel-count N] <프롬프트>` — 서브에이전트 유형 `orbit:mcp-subagent`을 띄운다. 접근 범위: MCP 사용 가능 — CLAUDE.md·스킬 없음. 하위 위임(Agent 도구) 불가 — 여러 에이전트로 나누는 일은 메인 세션이 한다.
...
2. 같은 턴에 `Agent` 도구로 띄운다. 개수가 N이면 N개를 한 턴에 병렬로:
   ...
   - `subagent_type`: `orbit:mcp-subagent`
   - `model`: 파싱한 모델
   - `prompt`: 프롬프트 원문
