---
description: 조사·대량 병렬·생성 작업의 기본 위임 대상(클린). CLAUDE.md·사용자 스킬·MCP 없이 위임 글만으로 일한다. `/orbit:clean-subagent` 커맨드가 사용한다.
argument-hint: "[모델명] [--parallel-count N] <프롬프트>"
---

# /orbit:clean-subagent 위임

`/orbit:clean-subagent [모델명] [--parallel-count N] <프롬프트>` — 서브에이전트 유형 `orbit:clean-subagent`을 띄운다. 접근 범위: CLAUDE.md·스킬·MCP 없음 — 위임 글만. 하위 위임(Agent 도구) 불가 — 여러 에이전트로 나누는 일은 메인 세션이 한다.
...
2. 같은 턴에 `Agent` 도구로 띄운다. 개수가 N이면 N개를 한 턴에 병렬로:
   ...
   - `subagent_type`: `orbit:clean-subagent`
   - `model`: 파싱한 모델
   - `prompt`: 프롬프트 원문
